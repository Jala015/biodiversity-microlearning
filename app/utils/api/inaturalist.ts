 ...
 */
export async function consultarApiINat(scientificName: string): Promise<ConsultaINatResult | null> {
  try {
...
export async function obterTaxonsIrmaos(
  ...
  try {
    ...
    const distractorCandidates = inatResp.results.filter(
...
export async function obterEspeciesAleatorias(
  ...
  try {
    ...
    const shuffled = inatResp.results.sort(() => 0.5 - Math.random());
    return shuffled.slice(0, count);
  } catch (error) {
