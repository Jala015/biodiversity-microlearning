...
import { obterEspeciesMaisComuns, montarDetalhesDasEspecies } from "~/utils/api_inat";
...
describe('API iNat', () => {
    it('should fetch species keys and counts from GBIF', async () => {
        ...
        expect(result.speciesKeys.length).toBeLessThanOrEqual(opcoes.maxSpecies * 2);
    });

    it('should build species details with media', async () => {
        const speciesKeys = ['8264893', '10886629'];
        const maxSpecies = 10;
        const counts = new Map<string, number>([['12345', 5], ['67890', 3]]);

        const details = await montarDetalhesDasEspecies(speciesKeys, maxSpecies, counts);
